...
export default function Home() {
  ...
  const [feed, setFeed] = useState<FeedState>({ synced: false, fixtureId: null, statKey: "—" });
  ...
  const matchedStake = stakes.find((s) => s.side === "OVER" || s.side === "UNDER");
  const opponentStake =
    stakes.length === 2
      ? stakes.find((s) => s.wallet !== (matchedStake?.wallet ?? "")) ?? null
      : null;
